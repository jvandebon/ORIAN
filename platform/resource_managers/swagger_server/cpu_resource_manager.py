...
class CPUResourceManager(rm.ResourceManager):

	def __init__(self, cfg_file):

		super(CPUResourceManager, self).__init__(cfg_file)
	...
	def _get_impls(self):
		...
		impls_metadata = [{'impl_name': i['impl_name'], 
				'task_name': i['task_name'], 
				'impl_rm': i['impl_rm']} for i in impls]
...
# below functions are all to build perfrmance models for the tasks and implemented supported by this node
	...
	def get_data_points(self, task):
		...
		profiles = [p for p in all_profiles if 'T' in p['config']]		
		tp_data_points = []
		io_data_points = []
		ns = []
		vols = []
	...
	def build_nbody_model(self, impl_name):
		# get all nbody data points for throughput and io
		tp_data, io_data, ns, vols = self.get_data_points("nbody")
		# build decaying exponential model for throughput
		tp_models = self.build_decexp_model(tp_data, ns, vols)
		# build linear model for io
		io_models = self.build_linear_model(io_data, ns, vols)
		# store tp model for each configuration
		for n in tp_models:
		...
		# store io model for each configuration
		for n in io_models:
	...
	def build_adp_model(self, impl_name):
		# get all adpredictor data points for throughput and io
		tp_data, io_data, ns, vols = self.get_data_points("adp")
		# build saturating log model for throughput
	...
	def build_align_model(self, impl_name):
		# get all align data points for throughput and io
